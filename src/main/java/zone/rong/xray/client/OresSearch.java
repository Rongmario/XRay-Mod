    ...
    public static void add(String oreIdent, String name, int color) // Takes a string of id:meta or oreName to add to our search list.
    {
        ...
        if (oreIdent.contains(":")) // Hopefully a proper id:meta string.
        {
            ...
            if (splitArray.length != 2) {
                ...
                String notify = String.format("[�aFgt XRay�r] %s is not a valid identifier. Try id:meta (example 1:0 for stone) or oreName (example oreDiamond or mossyStone)",
                        oreIdent);
        ...
        if (OresSearch.searchList.isEmpty()) {
            ...
            for (Map.Entry<String, OreInfo> entry : FgtXRay.oredictOres.entrySet()) {
                ...
                if (!tempOredict.containsKey(key)) {
                    System.out.printf("[Fgt XRay] Ore %s doesn't exist in dictionary! Deleting.%n", key);
                }
            }
            ...
            for (Map.Entry<String, OreInfo> entry : FgtXRay.oredictOres.entrySet()) {
                ...
                if (oreDictOres.size() < 1) {
                    System.out.printf("[Fgt XRay] Ore %s doesn't exist! Skipping. (We shouldn't have this issue here! Please tell me about this!)%n", key);
                    continue;
                }
                for (int i = 0; i < oreDictOres.size(); i++) {
                    ...
                    System.out.printf("[Fgt XRay] Adding OreInfo( %s, %d, %d, 0x%x, %b ) %n",
                            value.oreName,
            ...
            {
                System.out.printf("[Fgt XRay] Adding OreInfo( %s, %d, %d, 0x%x, %b ) %n", ore.oreName, ore.id, ore.meta, ore.color, ore.draw);
                temp.add(ore);
            }
